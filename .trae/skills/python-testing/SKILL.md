...
from rdbase import RequestBatch
...
from rdbase import validate_non_empty
...
from rdbase import format_size
...
def test_load_config_falls_back_when_env_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    """环境变量缺失时应回退到默认值。"""
    monkeypatch.delenv("RDBASE_TIMEOUT", raising=False)
    cfg = config.load_from_env()
...
from rdbase import discover_configs
...
from rdbase import parse_number
...
from rdbase import StorageError, load
...
```bash
# 默认跑非慢测试 + 覆盖率
uv run pytest -m "not slow" --cov=rdbase --cov-fail-under=95

# 全量（含慢测试）
uv run pytest --cov=rdbase --cov-branch --cov-report=term-missing

# HTML 报告（本地排查未覆盖行）
uv run pytest --cov=rdbase --cov-report=html
```
...
def test_main_window_title(qtbot) -> None:
    ...
    assert window.windowTitle() == "rdbase"
...
from rdbase.widgets.search_box import SearchBox

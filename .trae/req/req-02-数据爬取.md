...
- [ ] 31 模块骨架与 Scrapy/Django 集成：ingest app 注册、4 个数据模型（IngestTask/IngestFieldMapping/IngestLog/IngestAlert）+ migration、Scrapy 子进程启动器（engine.py）、base spider、run_ingest 管理命令跑通空 Spider
- [ ] 32 REST/JSON API 爬取器 + 字段映射 + 写入目标：api_spider + JSONPath 解析、pipelines 字段映射与 SQLAlchemy 批量写入（复用 ConflictStrategy 与 sync_service 方言 UPSERT）、手动执行 API、集成测试
